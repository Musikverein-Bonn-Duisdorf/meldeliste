...
' : '').'
'.(requirePermission('perm_editConfig') ? '
<li><b>Backup</b> – ZIP mit Datenbank (Versionsinfo) und hochgeladenen Dateien unter <code>uploads/</code> (Fotos, Dokumente, Verträge, Mail-Anhänge) herunterladen oder wieder einspielen; im Browser über <code>Backup</code> (Recht <b>Konfiguration bearbeiten</b>), per CLI mit <code>php cron.php CRONID backup</code>; große Archive und Restore ohne HTTP-Timeout: <code>php scripts/restoreBackup.php backup.zip --yes</code>. Neue ZIPs stellen <code>uploads/</code> als Snapshot wieder her; ältere ZIPs ohne Dateien ändern <code>uploads/</code> nicht. Automatisiert remote nur mit eigenem <code>$backupToken</code> in <code>config.php</code> (mind. 32 Zeichen) über <code>cron.php?id=…&amp;cmd=backup</code> — nicht mit dem allgemeinen Cron-ID. PHP-Laufzeit wird für Backup/Restore unbegrenzt; der Webserver kann lange Requests trotzdem beenden. Erfolgreiche Downloads erscheinen im <b>Log</b> als Info, fehlgeschlagene als Fehler</li>
